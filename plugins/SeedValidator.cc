...
void SeedValidator::beginRun(edm::Run const&, edm::EventSetup const& setup) {
...
void SeedValidator::endRun(edm::Run const&, edm::EventSetup const&) {
  LogTrace("TrackValidator") << "SeedValidator::endRun()";
  int w=0;
  for (unsigned int ww=0;ww<associators.size();ww++){
    for (unsigned int www=0;www<label.size();www++){
